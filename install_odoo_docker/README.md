...
```
✅ This also installs **NGINX Proxy Manager** and **Portainer CE** (optional, recommended for production).
...
- **`~/.odoo-docker-secrets.txt`**: append-only log of every instance's credentials, `600` permissions.
...
```
🔒  SECURITY REMINDER
    1. Save credentials from: ~/.odoo-docker-secrets.txt
...
2. Create a **Proxy Host**:
   ...
   - **Forward Port**: your `ODOO_PORT` (e.g. `8069`)
...
✅ No need to expose ports publicly — NPM handles HTTPS termination and WebSocket routing.
...
cd ~
rm -rf ~/odoo/your-instance-name   # removes db-data, config, addons, docker-compose.yml, .env
```

> ⚠️ `-v` deletes the `odoo-data` named volume (Odoo filestore/sessions) — that's not stored under `~/odoo/...` so a plain `rm -rf` alone won't touch it. You need **both** commands for a truly complete wipe. PostgreSQL's `db-data`, on the other hand, *is* a regular folder on disk, so `rm -rf` alone removes it (with or without `-v`).
...
| `No write permission for ~/odoo` | The **top-level** `~/odoo` folder ownership got changed (e.g. by a broad `chown -R`) | `sudo chown $(whoami):$(whoami) ~/odoo` (no `-R` — don't touch existing instances' internal permissions) |
