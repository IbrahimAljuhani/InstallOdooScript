...
if [[ "$NGINX_CHOICE" == "y" || "$NGINX_CHOICE" == "yes" ]]; then
    ...
    # Set global client_max_body_size in nginx.conf
...
# ============================================================================
# HTTP SERVER (Stage 1 - Before SSL)
...
EOF
    ...
    # Test configuration (MUST succeed at this stage)
    if ! sudo nginx -t; then
        print_error "Nginx configuration test failed! Check syntax with: sudo nginx -T"
    fi
    ...
    print_info "✅ Nginx Baseline (HTTP) configured successfully for $OE_USER."
    ...
    if [[ "$SSL_CHOICE" == "y" || "$SSL_CHOICE" == "yes" ]]; then
        ...
        if sudo certbot --nginx \
            ...
            # ✅ Verify final HTTPS config
            if sudo nginx -t; then
                print_info "✅ Final HTTPS configuration validated successfully."
            else
                print_warn "⚠️  Nginx config test failed after Certbot. Manual review recommended."
            fi
        else
